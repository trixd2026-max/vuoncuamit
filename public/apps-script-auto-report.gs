 ...
 * 3) Them doan saveReport vao doPost (xem comment)
 * 4) Chay ham setupDailyReportTrigger() (hoac menu Vuon Cua Mit)
 * 5) Deploy > New version webhook neu can
 *
...
/**
 * (Tuy chon) Trong doPost, truoc dong "Action khong ho tro", them:
 *
 * if (action === "saveReport") {
 *   var sh = ensureBaoCaoSheet_(ss, data.reportSheetName || "BaoCao");
 *   appendReportRow_(sh, data);
 *   return jsonOut_({ ok: true, action: "saveReport" });
 * }
 */
