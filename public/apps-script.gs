...
/** Khi Google redirect POST->GET, van xu ly update qua query param */
function doGet(e) {
  try {
    var p = (e && e.parameter) ? e.parameter : {};
    var action = String(p.action || "").trim();
    logVcm_("doGet.start", { action: action, orderId: p.orderId || "", status: p.status || "", keys: Object.keys(p) });
    if (action === "updateStatus" || action === "updateInternalNote" || action === "updateOrderInfo") {
      return doPost(e);
    }
    if (p.orderId && p.status && !p.name && !p.phone && !p.items) {
      if (!e.parameter) e.parameter = {};
      e.parameter.action = "updateStatus";
      return doPost(e);
    }
    return jsonOut_({ ok: false, error: "GET chi dung de updateStatus. Dung POST de tao don.", action: action || null });
  } catch (err) {
    logVcm_("doGet.FATAL", safeErr_(err));
    return jsonOut_({ ok: false, error: "Loi doGet: " + safeErr_(err) });
  }
}
