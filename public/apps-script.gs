...
function updateOrderStatus_(sheet, orderId, status) {
  ...
  for (var r = 0; r < ids.length; r++) {
    if (String(ids[r][0] || "").trim().toLowerCase() === want) {
      ...
      try { appendStatusLog_(sheet, row, prev, status); } catch (e) {}
      found++;
    }
  }
  return found > 0;
...
function doPost(e) {
  ...
  if (action) return jsonOut_({ ok: false, error: "Action khong ho tro: " + action });
  if (!hasCustomer && !hasItems) return jsonOut_({ ok: false, error: "Thieu du lieu don hang" });
  if (hasOrderId && !hasCustomer) return jsonOut_({ ok: false, error: "Thieu ten/SDT khach" });
  ...
  try { sendOrderEmail_(data); } catch (errN) {}
  try { if (alerts.length > 0) sendStockAlertEmail_(alerts, data.orderId || ""); } catch (err2) {}
